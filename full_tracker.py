"""
Same tracker but without the max-pooling before cross-correlation.
Larger correlation maps but much slower.
"""
...
class Vgg19:
    ...
    def upscale_n(self, bottom, name, n):
        _, w, h, _ = bottom.get_shape().as_list()
        return tf.image.resize_nearest_neighbor(bottom, size=[w * 2**n, h * 2**n], name=name)
